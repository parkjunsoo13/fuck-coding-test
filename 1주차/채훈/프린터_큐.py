...
for _ in range(T): # 테스트케이스 수만큼 루프
    _, target = map(int, input().split())
    q = deque() # 큐 초기화
    
    docs = list(map(int, input().split())) # 문서 중요도 입력
    for idx, doc in enumerate(docs): # 문서의 중요도와 들어온 순서를 리스트에 담아 큐에 삽입
        q.append([doc, idx])

    i = 1 # 몇 번째로 출력되는지 세어야 하므로 인덱스 변수 생성
    while q:
        if q[0][0] < max(q)[0]: # 문서의 중요도가 더 높은 문서가 있다면 큐에서 빼내어 다시 뒤로 삽입
            q.append(q.popleft())
        else:
            doc = q.popleft() # 중요도가 더 높은 문서가 없다면 큐에서 빼냄
            if doc[1] == target:  # 만약 빼낸 문서가 target이면 target문서가 몇 번째로 인쇄되는지 출력
                print(i)
                break
            i += 1
